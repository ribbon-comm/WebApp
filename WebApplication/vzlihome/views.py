...
def vzli(request):
    ...
    if request.method == 'POST':
        ...
        if (TargetCriteriaType and Target and TapID and emsip):
            host = "10.54.92.182"
            port = 22
            username = "rshah"
            password = "Rs@200694"
            command2 = "perl /home/rshah/LI/target_create_edited.pl {emsip} {criteriatype} {criteriaid} {tapid}".format(
                emsip=emsip, criteriatype=TargetCriteriaType, criteriaid=Target, tapid=TapID)
            print(command2)
        ...
        if messages == "Successful":
            return render(request, 'form_component_vzli.html', {'messages': messages, })
        else:
            return render(request, 'form_component_vzli.html', {'messages': messages, })

    else:
        return render(request, "form_component_vzli.html")

def vzli_liserver(request):
        messages = None
        global sbcip
        global randomlist
        global ip1
        ...
        if request.method == 'POST':
            ...
            if (batsusername and batspassword and int(msnum) < 16):
                ...
                for i in randomlist:
                    ...
                    if count == 3:
                        serversnum += 1
                        commandrun = '/ats/bin/liServer -si {sip} -sp {sport} -tmi {tip} -tmp {tport} -umi {uip} -ump {uport} -multimedia &'.format(
                            sip=ip1, sport=portlist.pop(), tip=ip1, tport=portlist.pop(), uip=ip1, uport=portlist.pop())
                        # print(commandrun)
            ...
            if messages == "Successful":
                return render(request, 'form_component_liServer_vzli.html', {'messages': messages, })
            else:
                return render(request, 'form_component_liServer_vzli.html', {'messages': messages, })
        else:
            return render(request, "form_component_liServer_vzli.html")


def vzli_configs(request):
    messages = ""
    global randomlist
    clilist = []
    global msnum
    ...
    if request.method == 'POST':
        ...
        cdccmd = 'set addressContext default intercept callDataChannel CDC interceptStandard {InterceptStandard} vendorId {vendorID} ipInterfaceGroupName {ipInterfaceGroup} mediaIpInterfaceGroupName {mediaipInterfaceGroup}'.format(
            vendorID=vendorID, InterceptStandard=InterceptStandard, ipInterfaceGroup=IpInterfaceGroupName,
            mediaipInterfaceGroup=IpInterfaceGroupName)
        ...
        if messages == "Successful":
            return render(request, 'form_component_liServer_vzliconfigs.html', {'messages': messages, })
        else:
            return render(request, 'form_component_liServer_vzliconfigs.html', {'messages': messages, })
    else:
        ...
        return render(request, "form_component_liServer_vzliconfigs.html", context)
...
def func(host, username, pswd, sbcip):
    ...
    curlString = "curl -k -i --user admin:Sonus@123 -X GET \"https://{mgtIP}/api/config/addressContext/default\" > 1.txt".format(
        mgtIP=str(sbcip))
    interact.send(curlString)
    interact.expect('> ')
    interact.send(
        "cat 1.txt | grep -P -o -e \"<ipInterfaceGroupName>.*(?=</ipInterfaceGroupName>)\" | cut -d \">\" -f 2")
    interact.expect('> ')
    output1 = interact.current_output_clean.strip()
